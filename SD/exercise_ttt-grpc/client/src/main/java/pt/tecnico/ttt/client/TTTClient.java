		...
		try (Scanner scanner = new Scanner(System.in)) {
			...
			do {
				...
				do {
					...
					if(go == 10){
						UltimaRequest req = UltimaRequest.newBuilder().build();
						int lrow = stub.ultimaJogada(req).getRow();
						int lcol = stub.ultimaJogada(req).getCol();
						int lplayer = stub.ultimaJogada(req).getPlayer();
						System.out.println("Last play was: row " + lrow + " col " + lcol + " by player " + lplayer);
					}
					...
					PlayRequest request = PlayRequest.newBuilder().setRow(row).setCol(column).setPlayer(player).build();
					play_res = stub.play(request).getRes();
				...
				winner = stub.checkWinner(request).getResult();
