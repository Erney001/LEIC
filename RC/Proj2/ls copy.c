  ...
  for(node_t i = first; i <= last; i++){
    ...
    if(msg->ls[i].version > actual_state->cost[i].version){
      ...
      for(node_t j = first; j <= last; j++){
        ...
        if(sender == 2 && current == 3){
          printf("%d ", actual_state->cost[i].link_cost[j]);
        }
      }
      if(sender == 2 && current == 3){
        printf("\n");
...
int dijkstra(state_t * actual_state, node_t first, node_t current, node_t last, node_t total_nodes, node_t sender){
  ...
  if(current == 0){
    ...
    for(node_t n = first; n <= last; n++){
      printf("%d: ", n);
      for(node_t nn = first; nn <= last; nn++){
        printf("%d ", actual_state->cost[n].link_cost[nn]);
      }
      printf("\n");
    }
    printf("**********************************************************************\n");
  ...
  for(node_t n = first; n < total_nodes; n++){
    if(n != current){
      ...
      for(node_t nn = first; nn < total_nodes; nn++){
        tmp_cost = COST_ADD(actual_state->cost[current].link_cost[nn], actual_state->cost[nn].link_cost[n]); /// ou get link cost no primeiro arg
        cost_t t;
        for(node_t nnn = first; nnn < total_nodes; nnn++){
          if(nn != nnn && nn != current){
            ...
            if(current == 0){
              //printf("tmp %d (%d) vs t %d (%d, %d) to %d\n", tmp_cost, nn, t, nn, nnn, n);
            }
            if(t < tmp_cost){
              tmp_cost = t;
        ...
        if(current == 0){
          for(node_t i = first; i <= last; i++){
            printf(">%d ", actual_state->next_hop[i]);
          }
          printf("\n");
        }
        ...
        if(get_link_cost(n) == COST_INFINITY && actual_state->cost[tmp_hop].link_cost[n] == COST_INFINITY){ // if not a neighbor and itself is hop, cancel route
          printf("B\n");
          continue;
        }
        if(tmp_cost < current_cost && get_link_cost(tmp_hop) != COST_INFINITY){
          ...
          tmp_hops[n] = tmp_hop; // hop so e nn se hop para nn for = a nn
        }
      }
    }
  }
  ...
  if(current == 0){
    ...
    for(node_t n = first; n <= last; n++){
      printf("%d: ", n);
      for(node_t nn = first; nn <= last; nn++){
        printf("%d ", actual_state->cost[n].link_cost[nn]); // tmp_costs[n].link_cost[nn]
      }
      printf("\n");
    }
    printf("**********************************************************************\n");
  ...
  for(node_t n = first; n < total_nodes; n++){
    if(tmp_costs[current].link_cost[n] != actual_state->cost[current].link_cost[n]){
      //printf("prev %d\n", actual_state->cost[current].link_cost[n]);
