...
export default class RemoteServices {
    ...
    return httpClient
      ...
      .then((response) => {
        return response.data.map((weeklyScore: any) => {
          return new WeeklyScore(weeklyScore); // confirm this
        });
      })
      .catch(async (error) => {
        throw Error(await this.errorMessage(error));
  ...
  static async removeWeeklyScore(dashboardId: number, weeklyScoreId: number) {
    return httpClient
      .delete(
        '/students/dashboards/' + dashboardId + '/weeklyScores/' + weeklyScoreId
      )
      .catch(async (error) => {
        throw Error(await this.errorMessage(error));
      });
  ...
  static async getFailedAnswers(dashboardId: number): Promise<FailedAnswer[]> {
    return httpClient
        ...
        .then((response) => {
          return response.data.map((failedAnswers: any) => {
            return new FailedAnswer(failedAnswers);
          });
        })
        .catch(async (error) => {
          throw Error(await this.errorMessage(error));
