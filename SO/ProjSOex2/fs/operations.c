...
int lookup(char *name, int ** inumber_table, int * number_ofDirs, int callingFunc) {
	...
	int current_inumber = FS_ROOT, i = 0;
	type nType;
	union Data data;
	...
	if (callingFunc == LOOKUP) unlockThreads(*inumber_table, i);
...
int move(char *name1, char *name2) {
	...
	// dest_child e a dir final, parent dessa dir final fica em paths[PATH2]
	...
	split_parent_child_from_path(pathCopy, &parent_name, &child_name);
	// child_name e a dir inicial, parent dessa dir final fica em paths[PATH1]
	...
	if (path1_parent_inumber == FAIL || path2_parent_inumber == FAIL) {
		unlock_move_resources(paths, pathCopy, dest_child);
		unlockThreads(inumber_table, number_ofDirs);
		return FAIL;
	}
