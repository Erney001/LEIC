...
int inode_create(type nType) {
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        /* Se nao usar o trylock nao temos deadlocks, mas sei que é necessario dar aqui locks.
           Com o mutex isto nao acontece, mas sei que nao é correto deixar com mutex.
         
          Por vezes uma diretoria é criada, mas a thread acaba por nao sair do create.
          No entanto mais tarde é feito um move com essa tal diretoria, e o move é efetuado
        devidamente... Pode observar no output, se quiser, se faz favor.

          Nao compreendo porque, vou deixar isto pois é o mais correto, nao consigo
         encontrar a razao deste bug estranho.
        */

        int lock_result = pthread_rwlock_trywrlock(&inode_table[inumber].rwlock);

        if (lock_result == EBUSY) {
            continue;
        }

        else if (lock_result != EBUSY && lock_result != 0) {
        ...
        if (inode_table[inumber].nodeType == T_NONE) {
            ...
            if (pthread_rwlock_unlock(&inode_table[inumber].rwlock) != 0)
                perror("Error unlocking inode in inode_create\n");
...
*   - rwlock: pointer to a rwlock
 * Returns: SUCCESS or FAIL
 */
int inode_get(int inumber, type *nType, union Data *data, pthread_rwlock_t * rwlock) {
    /* Used for testing synchronization speedup */
    insert_delay(DELAY);
    ...
    if (rwlock)
        ...
        *rwlock = inode_table[inumber].rwlock;
